...
class recognition:
    # oriImage
    def open(self):
        img_path = askopenfilename(initialdir='./testImage', title='选择待识别图片', filetypes=[("jpg", "*.jpg"), ("png", "*.png")])
        if img_path:
    ...
    def getROI(self, img):
        ...
        img = cv2.morphologyEx(img, cv2.MORPH_CLOSE, kernel)  # 闭操作，横向涂抹
        # imshow("close",img)
        # kernel = ones((4, 25), uint8)
        img = cv2.morphologyEx(img, cv2.MORPH_OPEN, kernel)  # 开操作，去除细线
        # imshow("open2",img)
