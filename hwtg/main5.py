...
def pretreatment(filename, resize_rate=1):
    ...
    for i,rect in enumerate(car_contours):
        ...
        if left_point[1] <= right_point[1]:  # 正角度
            ...
            pts2 = np.float32([left_point, new_heigth_point, new_right_point])  # 字符只是高度需要改变
            ...
            pts2 = np.float32([new_left_point, new_heigth_point, right_point])  # 字符只是高度需要改变
            pts1 = np.float32([left_point, heigth_point, right_point])
            M = cv2.getAffineTransform(pts1, pts2)
            dst = cv2.warpAffine(oldimg, M, (pic_width, pic_height))
